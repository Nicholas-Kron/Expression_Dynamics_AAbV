...
printf "sample\ttotal\tforward\treverse\n" > ${projdir}/alignment_stats/${experiment}_host_twitchin_read_orientations.txt
...
#
samtools view -b -f 128 -F 16 ${samp}_host_Aligned_twitchin.bam > ${samp}_fwd1.bam
samtools index ${samp}_fwd1.bam

samtools view -b -f 80 ${samp}_host_Aligned_twitchin.bam > ${samp}_fwd2.bam
...
#
samtools view -b -f 144 ${samp}_host_Aligned_twitchin.bam > ${samp}_rev1.bam
samtools index ${samp}_rev1.bam

samtools view -b -f 64 -F 16 ${samp}_host_Aligned_twitchin.bam > ${samp}_rev2.bam
...
total=`samtools view -c -F 260 ${samp}_host_Aligned_twitchin.bam`
