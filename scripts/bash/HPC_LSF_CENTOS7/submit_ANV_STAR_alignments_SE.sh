...
#check for required input files
  [[ ! -f ${projdir}/cleaned_reads/"${samp}".repaired.fastq.gz ]] && ( printf "cleaned read files for "${samp}" not found!" && exit )
...
--runThreadN 16 \
--genomeDir ${index} \
--readFilesIn ${projdir}/cleaned_reads/${samp}.repaired.fastq.gz \
